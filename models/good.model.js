...
export class GoodDAO {
	static async insert(data) {
		...
		const result = await pgdb.any( 
			`INSERT INTO ${Good.schema}.${Good.database} (${key.join(',')}) VALUES (${[...Array(key.length).keys()].map((_,i)=>"$"+(i+1))})`,
	...
	static async update(data, id) {
        ...
        console.log('sql ', `UPDATE ${Good.schema}.${Good.database} SET ${item.toString()} WHERE id=${data.id}`)
        let result = await pgdb.query(`UPDATE ${Good.schema}.${Good.database} SET ${item.toString()} WHERE id=${data.id}`,
	...
	static async delete(id) {
		let result = await pgdb.any(`UPDATE ${Good.schema}.${Good.database} SET isdeleted=true WHERE id=${id}`)
		return result;
	}

	//不够抽象化
	static async search(body) {
		let result = await pgdb.any(`SELECT * FROM ${Good.schema}.${Good.database} WHERE isdeleted=false AND 
